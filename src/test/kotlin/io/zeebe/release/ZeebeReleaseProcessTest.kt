...
class ZeebeReleaseProcessTest {
    ...
    fun beforeEach() {
        ...
        deployFakeChildProcess("zeebe-post-release", hashMapOf(
            "release_date" to "date and time(\"${dateFormatter.format(ZonedDateTime.now().plusHours(1))}\")",
    ...
    fun `should be able to complete the instance`() {
        ...
        // then
        testHelper.assertThatUserTaskActivated(instanceEvent.processInstanceKey, "create-release-branch")
        testHelper.completeUserTask(instanceEvent.processInstanceKey, "create-release-branch")
        testHelper.assertThatUserTaskActivated(instanceEvent.processInstanceKey, "create-code-freeze-backport-label")
        testHelper.completeUserTask(instanceEvent.processInstanceKey, "create-code-freeze-backport-label")
        testHelper.assertThatUserTaskActivated(instanceEvent.processInstanceKey, "collect-release-notes")
        testHelper.completeUserTask(instanceEvent.processInstanceKey, "collect-release-notes")
        testHelper.assertThatCalledProcessActivated(instanceEvent.processInstanceKey, "zeebe-release-qa")
        testHelper.assertThatCalledProcessCompleted(instanceEvent.processInstanceKey, "zeebe-release-qa")
        ...
        testHelper.assertThatUserTaskActivated(instanceEvent.processInstanceKey, "release-on-maven-central")
        testHelper.completeUserTask(instanceEvent.processInstanceKey, "release-on-maven-central")
        testHelper.assertThatCalledProcessActivated(instanceEvent.processInstanceKey, "zeebe-post-release")
        testHelper.assertThatCalledProcessCompleted(instanceEvent.processInstanceKey, "zeebe-post-release")
    ...
    private fun deployFakeChildProcess(name: String, outputVariables: Map<String, String> = emptyMap()) {
